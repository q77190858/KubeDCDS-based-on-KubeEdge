...
func (e *edgestream) Start() {
	...
	//juju add: 保存tls密钥用于解密
	w, err := os.OpenFile("/etc/kubeedge/edgeStreamKey.txt", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		klog.Infof("failed to open file edgeStreamKey.txt err %+v", err)
		return
	}
	defer w.Close()
