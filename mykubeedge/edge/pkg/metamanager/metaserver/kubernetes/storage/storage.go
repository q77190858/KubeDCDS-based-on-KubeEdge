...
func getIPStrFromContext(ctx context.Context) string {
	for v := reflect.ValueOf(ctx).Elem(); v.Type().String() != "context.emptyCtx"; v = v.FieldByName("Context").Elem().Elem() {
		if v.Type().String() == "context.valueCtx" {
			...
			// fmt.Printf("key: %#v val: %#v\n", v.FieldByName("key"), val)
			if val.Elem().Type().String() == "*net.TCPAddr" {
				ip := fmt.Sprint(val.Elem().Elem().FieldByName("IP"))
				ip = ip[1 : len(ip)-1]
				// klog.Infof("get request ip: %s\n", ip)
				return ip
			}
		}
	}
...
func (r *REST) Get(ctx context.Context, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	// klog.Infof("ctx: %+v, info: %+v, options: %+v", ctx, info, options)
	IPStr := getIPStrFromContext(ctx)
	if IPStr == "" {
		klog.Errorf("fail to get request ip")
	}
	klog.Infof("request ip: %v", IPStr)
	//request ip is not self local request, return storage data
...
func (r *REST) Watch(ctx context.Context, options *metainternalversion.ListOptions) (watch.Interface, error) {
	...
	if !strings.Contains(IPStr, "127 0 0 1") {
		if !kefeatures.DefaultFeatureGate.Enabled(kefeatures.RequireAuthorization) {
			watch, err := r.Store.Watch(ctx, options)
			if err == nil {
				klog.Infof("[metaserver/reststorage] success to get a approved application for watch(%v) from local storage without Authorization", info.Path)
				return watch, err
			}
		}
	}
