...
func NewMetaServer(nodeIP string) *MetaServer {
	ls := MetaServer{
...
func (ls *MetaServer) startHTTPServer(stopChan <-chan struct{}, quit <-chan int, isPublic bool) {
...
func (ls *MetaServer) startHTTPSServer(stopChan <-chan struct{}, quit <-chan int, isPublic bool) {
	_, err := ls.getCurrent()
	if err != nil {
...
func (ls *MetaServer) Start(stopChan <-chan struct{}, quit <-chan int, isPublic bool) {
	if kefeatures.DefaultFeatureGate.Enabled(kefeatures.RequireAuthorization) {
		ls.startHTTPSServer(stopChan, quit, isPublic)
	} else {
		ls.startHTTPServer(stopChan, quit, isPublic)
	}
}
